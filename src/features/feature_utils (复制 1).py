...
class FeatureBuilder:
    ...
    def _resize_features(self, features: Dict[str, np.ndarray], target_dim: int) -> Dict[str, np.ndarray]:
        """
        Resize features to target dimension.
        
        Args:
            features: Dictionary mapping entity IDs to features.
            target_dim: Target dimension.
            
        Returns:
            Dictionary mapping entity IDs to resized features.
        """
        # Get current dimension
        current_dim = list(features.values())[0].shape[0]
        ...
        if current_dim > target_dim:
            ...
            # Padding
            padded_matrix = np.zeros((feature_matrix.shape[0], target_dim))
